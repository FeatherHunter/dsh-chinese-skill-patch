...
export function apply(ctx, config) {
        ...
        const provider = {
            ...
            async list(options = {}) {
                ...
                for (const root of roots) {
                    let entries = [];
                    try {
                        entries = await fs.readdir(root.dir, { withFileTypes: true, encoding: 'utf8' });
                    }
                    catch {
                        continue;
                    }
    ...
    ctx.effect(() => {
        ctx.tools.register(defineTool({
            ...
            parameters: {
                keyword: { type: 'string', required: false, description: '可选关键词过滤' },
            },
            output: {
            ...
            async execute(args) {
                ...
                const filtered = args.keyword && typeof args.keyword === 'string'
                    ? list.filter((s) => s.name.includes(args.keyword))
                    : list;
                return {
    ...
    ctx.logger.info?.(`[dsh-chinese-skill-patch] installed; SKILL_NAME regex relaxed to /^[\\p{L}0-9]+(?:-[\\p{L}0-9]+)*$/u`);
}
