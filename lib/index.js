...
function validateRuntimeSkillCN(skill) {
    if (!CN_KEBAB.test(skill.name))
...
function validateDefinitionCN(skill) {
    const n = skill.name;
    if (typeof n !== 'string')
        throw new TypeError('loaded skill name must be a string');
    if (!CN_KEBAB.test(n))
        throw new Error(`loaded skill has invalid name "${n}"`);
    if (typeof skill.description !== 'string')
        throw new TypeError(`loaded skill "${n}" description must be a string`);
    if (skill.description.length === 0)
        throw new Error(`loaded skill "${n}" requires a description`);
    validateInvocation(skill.invocation, `loaded skill "${n}"`);
    if (skill.whenToUse !== undefined && typeof skill.whenToUse !== 'string')
        throw new TypeError(`loaded skill "${n}" whenToUse must be a string`);
    if (typeof skill.source !== 'string')
        throw new TypeError(`loaded skill "${n}" source must be a string`);
    if (typeof skill.provider !== 'string')
        throw new TypeError(`loaded skill "${n}" provider must be a string`);
    if (typeof skill.content !== 'string')
        throw new TypeError(`loaded skill "${n}" content must be a string`);
    if (skill.path !== undefined && typeof skill.path !== 'string')
        throw new TypeError(`loaded skill "${n}" path must be a string`);
}
function toError(e) {
    try {
    ...
    try {
        ...
        return new Error('[unrenderable thrown value]');
    }
}
...
function waitWithAbort(promise, signal) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const onAbort = () => { cleanup(); reject(toError(signal.reason)); };
...
function normalizeProviderObservation(output, providerName) {
    ...
    if (!Array.isArray(output.candidates) || typeof output.complete !== 'boolean')
        throw new TypeError(`skill provider "${providerName}" list() must return an array or { candidates, complete } observation`);
    return output;
}
...
async function patchFilesOnDisk(ctx) {
    try {
        ...
        async function resolveFile(id) {
            ...
            const bases = [];
            try {
                bases.push(path.join(os.homedir(), 'AppData', 'Roaming', 'DSH Desktop', 'agent', 'node_modules', '@deepseek-ai', pkg, 'lib', 'index.js'));
            }
            catch { }
            try {
                bases.push(path.join(os.homedir(), '.dsh', 'agent', 'node_modules', '@deepseek-ai', pkg, 'lib', 'index.js'));
            }
            catch { }
            const envAgent = process.env.AGENT_RUNTIME ?? process.env?.AGENT_RUNTIME;
            if (envAgent)
                bases.push(path.join(String(envAgent), 'node_modules', '@deepseek-ai', pkg, 'lib', 'index.js'));
            for (const drive of ['C:', 'D:']) {
                try {
                    const user = path.basename(os.homedir());
                    if (user)
                        bases.push(path.join(drive, 'Users', user, 'AppData', 'Roaming', 'DSH Desktop', 'agent', 'node_modules', '@deepseek-ai', pkg, 'lib', 'index.js'));
                }
                catch { }
            }
        ...
        const tryPatch = async (id, replacements, alreadyPatchedMarker) => {
...
async function parseSkillFileCN(filePath, ctx, signal) {
    const fs = await import('node:fs/promises');
    let raw;
    try {
        raw = await fs.readFile(filePath, { encoding: 'utf8', signal });
    ...
    try {
        const dm = data['disable-model-invocation'];
...
export function apply(ctx, config) {
    ...
    ctx.effect(() => {
        ...
        // Patch get
        proto.get = async function (name, options = {}) {
            if (!CN_KEBAB.test(name))
                return undefined;
        ...
        proto.listLayerCandidates = async function (layer, options) {
            throwIfAborted(options.signal);
            const candidates = [];
            let cacheable = true;
            let runtimeOrder = 0;
            for (const skill of [...layer.runtime.values()].sort((a, b) => compareCodePoints(a.name, b.name))) {
                candidates.push({
                    ...
                    provider: { name: RUNTIME_PROVIDER, list() { return Promise.resolve([]); }, get(candidate) { return Promise.resolve(candidate.locator); } },
                    providerOrder: -1,
            ...
            // providers
            const providers = [...layer.providers.values()];
            for (const { provider, order } of providers) {
                ...
                for (const candidate of observation.candidates) {
                    try {
                        ...
                        this.ctx.logger.warn(`skill provider "${provider.name}" returned invalid skill name "${candidate.name}" ignored (cn-patch)`);
                        continue;
                    }
    ...
    ctx.effect(() => {
        const provider = {
            ...
            async list(options = {}) {
                ...
                // project roots
                try {
                    const projectRoot = await findProjectRoot(cwd, ctx.get?.('fs'));
                ...
                const extraBase = config?.extraAgentsDir ?? process.env.DSH_AGENTS_DIR;
                if (extraBase) {
                    try {
                        const fs = await import('node:fs/promises');
                        const ents = await fs.readdir(extraBase, { withFileTypes: true }).catch(() => []);
                        for (const e of ents) {
                            if (!e.isDirectory())
                                continue;
                            ...
                            if (!roots.some(r => r.path === p)) {
                                try {
                                    ...
                                    if (st.isDirectory())
                                        roots.push({ path: p, source: 'project-dsh', rank: 100 });
                                }
                                catch { }
                            }
                        }
                ...
                for (const root of roots) {
                    ...
                    for (const ent of entries) {
                        if (root.skipSystem && ent.name === '.system')
                            continue;
                        try {
                            let locator;
                            let skillPath;
    ...
    ctx.effect(() => {
        ...
        // 关键：仅以内存中 isSkillName 为准，避免“文件已含 \\p{L} 但内存仍是旧 ascii 正则”导致的重启前 0 次注入窗口（审查 #1 指出的假阳性）
        let nativeSupportsCNCache;
        async function nativeSupportsCN() {
            if (nativeSupportsCNCache !== undefined)
                return nativeSupportsCNCache;
            try {
                const mod = await import('@deepseek-ai/dsh-skill');
                if (mod?.isSkillName) {
                    const cn1 = mod.isSkillName('备忘录');
                    const cn2 = mod.isSkillName('私家大厨');
                    const ascii = mod.isSkillName('test-skill');
                    // 原生已支持 CN 当且仅当中文与 ascii 均通过
                    if (cn1 && cn2 && ascii) {
                        nativeSupportsCNCache = true;
                        return true;
                    }
                    // 中文失败 => 明确不支持，快速 fallback
                    if (!cn1 || !cn2) {
            ...
            // import 失败或 isSkillName 缺失时，保守认为未支持，保留插件兜底，避免 0 注入
            nativeSupportsCNCache = false;
            return false;
        }
        const off = ctx.on('agent/pre-step', async ({ agent, messages, signal }, next) => {
            ...
            const alreadyInjected = new Set((decision.messages ?? [])
            ...
            let renderSkillContent;
            let createUserMessage;
            try {
                const modSkill = await import('@deepseek-ai/dsh-skill');
                renderSkillContent = modSkill.renderSkillContent;
            }
            catch { }
            try {
                const modLlm = await import('@deepseek-ai/dsh-llm');
                createUserMessage = modLlm.createUserMessage;
            }
            catch { }
            if (!renderSkillContent || !createUserMessage)
                return decision;
            for (const n of names) {
                if (alreadyInjected.has(n))
                    continue;
                ...
                if (!CN_KEBAB.test(n))
                    continue;
                const skill = await ctx.skills.get(n, lookup).catch(() => undefined);
                signal?.throwIfAborted();
                if (!skill)
                    continue;
    ...
    ctx.effect(() => {
        ...
        try {
            const skillTool = defineTool({
                ...
                parameters: { name: { type: 'string', required: true, description: 'The exact skill name from the available skills list.' } },
                output: {
                    schema: {
                        type: 'object',
                    ...
                    render: (_args, value) => [{ type: 'text', text: value.content ?? JSON.stringify(value) }],
                },
                async execute(args, exec) {
                    if (!CN.test(args.name))
                    ...
                    const summary = (await skills.list(lookup)).find((s) => s.name === args.name);
                    if (!summary)
                        throw new Error(`skill "${args.name}" is unknown or no longer available`);
                    if (summary.invocation && summary.invocation.modelInvocable === false)
            ...
            const dispose = ctx.tools.register(skillTool);
